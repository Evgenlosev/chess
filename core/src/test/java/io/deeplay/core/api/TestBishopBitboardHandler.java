...
public class TestBishopBitboardHandler {
    ...
    public void testBishopInMiddlePosition() {
        ...
        assertEquals(expectedMoveInfoSet, BitboardHandler.getBishopMoves(FENBoard, new Coord(BitUtils.BitIndex.E6_IDX.ordinal())));
    }

    @Test
    public void testBishopInMiddlePositionClamped() { // слон зажат
        FENBoard FENBoard = new FENBoard("8/8/8/R3Q3/1k6/2b5/1r1p1K2/8 w - - 0 1"); // черные
        Set<MoveInfo> expectedMoveInfoSet = Stream.of(
        ...
        assertEquals(expectedMoveInfoSet, BitboardHandler.getBishopMoves(FENBoard, new Coord(BitUtils.BitIndex.C3_IDX.ordinal())));
    }

    @Test
    public void testBishopsInCornersPosition() {
        FENBoard FENBoard = new FENBoard("7k/8/8/8/8/8/7b/B3K3 b - - 0 1"); // 0 - белый (в углу), 15 - черный (у одного края)
        Set<MoveInfo> expectedWhiteMoveInfoSet = Stream.of( // белый слон A1 - 0 клетка
                ...
                .collect(Collectors.toSet());
        assertEquals(expectedWhiteMoveInfoSet, BitboardHandler.getBishopMoves(FENBoard, new Coord(BitUtils.BitIndex.A1_IDX.ordinal())));
