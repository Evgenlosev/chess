...
//                simpleLogicAppeal.getMoves(fenBoard.getFenNotation())
//                        .stream().map(MoveInfo::getCellTo).collect(Collectors.toList());
        ...
        */
        ...
        fenBoard = new FENBoard("4r1k1/1pp4p/p7/B5p1/1PQ5/5P1b/P5PP/3K1R2 w - - 0 1");
        ...
        sideBitboards = FENParser.parseFENToBitboardsOld(fenBoard.getFenNotation());
        ...
        allCoords = BitboardHandler.getAllPossibleMoves(fenBoard, Side.WHITE)
                .values().stream().map(MoveInfo::getCellTo).collect(Collectors.toList());
//        allCoords = simpleLogicAppeal.getMoves(fenBoard.getFenNotation())
//                .stream().map(MoveInfo::getCellTo).collect(Collectors.toList());
        uniqueCoords = new HashSet<>(allCoords);

        assertEquals(36, allCoords.size()); // TODO: не учитывается что короля не сможет походить под линию атаки ладьи сверху
