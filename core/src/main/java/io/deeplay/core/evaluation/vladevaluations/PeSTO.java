 ...
 * Интерполирует переход между этапами игры.
    ...
    private static int figureToInt(final Figure figure) {
        switch (figure) {
            ...
            case B_KING:
                return 11;
            default:
                throw new IllegalArgumentException("Figure is unrecognized: " + figure);
        }
    }
    ...
    void initTables() {
        ...
        for (int p = 0, pc = 0; p <= 5; pc += 2, p++) {
            for (sq = 0; sq < 64; sq++) {
                ...
                mgTable[pc + 1][sq] = mgValue[p] + mgPestoTable[p][PeSTO.flip(sq)];
                egTable[pc + 1][sq] = egValue[p] + egPestoTable[p][PeSTO.flip(sq)];
            }
        }
    }
